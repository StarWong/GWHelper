...
function IMAGE_FIRST_SECTION( const ntheader ): PImageSectionHeader;
begin
  Result := PImageSectionHeader(PByte(@ntheader) +
    UIntPtr(@PImageNtHeaders(nil)^.OptionalHeader) +
     PImageNtHeaders(@ntheader)^.FileHeader.SizeOfOptionalHeader);
...
begin
  ...
  nameRef := Pointer(PByte(CodeBase) + DWORD(exportDir.AddressOfNames));
  ordinal := Pointer(PByte(CodeBase) + DWORD(exportDir.AddressOfNameOrdinals));
  Idx := -1;
  ...
  temp := Pointer(PByte(CodeBase) + DWORD(exportDir.AddressOfFunctions) + Idx*4);
